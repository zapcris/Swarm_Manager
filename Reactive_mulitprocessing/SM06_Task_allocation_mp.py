...
class Task_Allocator_agent:
    ...
    def step_allocation(self, task_for_allocation, product_obj, data_opcua, T_robot):
        ...
        for i, (task, prod) in enumerate(zip(task_for_allocation, product_obj)):
            ...
            for j, tr in enumerate(T_robot):
                # print(i,j)
                self.broadcast_bid(j, task, data_opcua, T_robot)
            self.assign_bid(task, prod, i, T_robot)
            # print(self.bid_data)
        # task_for_allocation.append(Task(0, 0, [0, 0], 0, 0, False, "Final_Task", 999))
        return task_for_allocation, product_obj

    def normal_allocation(self, task_for_allocation, product_obj, T_robot):
        # print("Normal allocation started")
        self.bid_data = []
        for i, tr in enumerate(T_robot):
            # print("First allocation ",i)

            self.broadcast_bid(i, task_for_allocation)
            # print("Broadcast robot no", i+1)
        self.assign_bid(task_for_allocation, product_obj, 0)
        return task_for_allocation, product_obj

    def broadcast_bid(self, i, task, data_opcua, T_robot):
    ...
    def assign_bid(self, task, product, i, T_robot):
